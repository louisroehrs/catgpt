...
agent = RealtimeAgent(
    name="Assistant",
    instructions="You are a helpful cat assistant. Keep responses brief and friendly.",
    tools=[get_weather],
)
...
class NoUIDemo:
    def __init__(self) -> None:
        ...
        self.output_queue: queue.Queue[Any] = queue.Queue(maxsize=100)  # Buffer more chunks
        ...
        try:
            if event.type == "agent_start":
                ...
                try:
                    ...
                    if self.output_queue.qsize() > 0:  # Keep some buffer
                        try:
                            self.output_queue.get_nowait()
                            self.output_queue.put_nowait(np_audio)
                        except queue.Empty:
                ...
                pass
            # print(f"Raw model event: {_truncate_str(str(event.data), 5000)}")
            else:
                print(f"Unknown event type: {event.type}")
        except Exception as e:
            print(f"Error processing event: {_truncate_str(str(e), 5000)}")
